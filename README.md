...
- **感情パラメータ入力**: 5つの感情（Joy, Calm, Anger, Sadness, Fear）を1〜5のスケールでWeb UI（ブラウザ）またはターミナル（CLI）から指定可能。
...
### Web UI経由で実行する（推奨）
ターミナルでWebサーバーを起動し、MacBookのブラウザからアクセスして操作できます。
...
┌─────────────────────────────────────────────────────────┐
│                    ユーザーインターフェース                     │
│  ブラウザ (localhost:8000) ────► MacBook (Flask :8000)       │
│                            または                            │
│                   ターミナル (CLI)                            │
└────────────────────────┬────────────────────────────────┘
                         │ 感情パラメータ
                         │ (Joy / Calm / Anger / Sadness / Fear: 各1〜5)
                         ▼
┌─────────────────────────────────────────────────────────┐
│                      AI 生成パイプライン                       │
│                                                         │
│  [1] Google Gemini 2.5 Flash                            │
│      感情パラメータ → 3D生成用テキストプロンプト                   │
│                         │                               │
│                         ▼                               │
│  [2] Tripo AI API (text_to_model)                       │
│      テキストプロンプト → 3Dモデル (.glb) ダウンロード            │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │ .glb ファイル
                         ▼
...
│                     ローカル後処理                            │
│                                                         │
│  [3] Blender (バックグラウンド実行)                            │
│      .glb インポート → リサイズ → .stl エクスポート              │
│                       └──► アニメーション動画レンダリング (.webm) │
│                         │                               │
│                         ▼                               │
│  [4] PrusaSlicer (バックグラウンド実行)                        │
│      .stl → G-Code 生成                                  │
│                                                         │
└────────────────────────┬────────────────────────────────┘
                         │ G-Code ファイル
...
│                      3D プリント出力                          │
│                                                         │
│  [5] OctoPrint (REST API)                               │
│      G-Code アップロード → 印刷ジョブ自動開始                    │
│                         │                               │
│                         ▼                               │
│              接続された 3D プリンター                           │
│                                                         │
...
| ステップ | 入力 | 処理 | 出力 |
|---|---|---|---|
| 1 | 感情パラメータ (5値) | Gemini 2.5 Flash | 3D生成プロンプト (テキスト) |
| 2 | 3D生成プロンプト | Tripo AI API | 3Dモデル (`.glb`) |
| 3 | `.glb` | Blender | `.stl` + 動画 (`.webm`) |
| 4 | `.stl` | PrusaSlicer | G-Code (`.gcode`) |
| 5 | G-Code | OctoPrint API | 3Dプリント開始 |
